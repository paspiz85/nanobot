...
public class Attack4SideParallelHalf2Wave extends Attack {
	...
	private static final Attack4SideParallelHalf2Wave instance = new Attack4SideParallelHalf2Wave();
	...
	protected void doDropUnits(int[] attackGroup) throws InterruptedException {
		...
		for (int wave = 0; wave < 2; wave++) {
			for (int unitIdx = 0; unitIdx < attackGroup.length; unitIdx++) {
				...
				// select unit
				Robot.instance().leftClick(Clickable.getButtonAttackUnit(unitIdx + 1), 100);
				...
				for (int i = 0; i < topToRightPoints.length; i++) {
					...
					Robot.instance().leftClick(topRightPoint[0], topRightPoint[1], PAUSE_BETWEEN_UNIT_DROP);
					if (i < topToLeftPoints.length) {
						int[] topLeftPoint = topToLeftPoints[i];
						Robot.instance().leftClick(topLeftPoint[0], topLeftPoint[1], PAUSE_BETWEEN_UNIT_DROP);
					}
				}
			}
		}
		for (int wave = 0; wave < 2; wave++) {
			for (int unitIdx = 0; unitIdx < attackGroup.length; unitIdx++) {
				...
				for (int i = 0; i < rightToBottomPoints.length; i++) {
					...
					Robot.instance().leftClick(rightToBottomPoint[0], rightToBottomPoint[1], PAUSE_BETWEEN_UNIT_DROP);
					int[] leftToBottomPoint = leftToBottomPoints[i];
					Robot.instance().leftClick(leftToBottomPoint[0], leftToBottomPoint[1], PAUSE_BETWEEN_UNIT_DROP);
				}
			}
		}
	}
